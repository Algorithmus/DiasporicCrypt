...
func _fixed_process(delta):
	...
	if (cycle == "attack"):
		...
		set_global_pos(Vector2(512 + x, y - 848))
		...
		if (inversehex):
			...
			if (current_inversehex_delay >= INVERSEHEX_DELAY):
				...
				inversehex_obj.set_global_pos(Vector2(offsetx + inversehex_index * 320 * inversehex_direction, -1376))
		...
		exporb.set_value(ep)
		exporb.set_global_pos(Vector2(512, -592))
...
func create_projectile(ratex, ratey, angle, direction):
	...
	var x = 112 * cos(angle)
	var y = 112 * sin(angle)
	projectile.set_global_pos(Vector2(get_global_pos().x + x, get_global_pos().y + y))
...
func check_damage():
	...
	for tile in tiles:
		if (tile.get_name() == "sunbeam"):
			...
			var hitpos = hp_obj.calculate_hitpos(tile.get_global_pos(), tile.get_child(0).get_shape().get_extents(), get_global_pos(), collision_rect.get_child(0).get_shape().get_extents())
