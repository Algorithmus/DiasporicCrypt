...
func check_attack():
	...
	if (can_attack() && ai_obj.get("input") == "attack"):
		...
		var offsetX = player.get_node("player").get_global_pos().x - (get_global_pos().x + projectile_offset.x)
		var offsetY = player.get_node("player").get_global_pos().y - (get_global_pos().y + projectile_offset.y)
