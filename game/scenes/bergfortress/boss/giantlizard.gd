...
func activate():
	...
	for armor_obj in armor.get_children():
		armor_obj.set_fixed_process(true)
...
func step_target():
	...
	if (cycle == "walk"):
		...
		new_animation = "idle"
		set_global_pos(Vector2(get_global_pos().x + direction*walkspeed, get_global_pos().y))
		if (get_global_pos().x > origin.x + patrolrange * 32 || get_global_pos().x < origin.x - patrolrange * 32):
			direction = direction * -1
			set_scale(Vector2(direction, 1))
		current_attack_delay += 1
		if (current_attack_delay >= ATTACK_COOLDOWN):
			var startRange = 6*32*direction + get_global_pos().x
			var playerR = player.get_global_pos().x + player.sprite_offset.x
			var playerL = player.get_global_pos().x - player.sprite_offset.x
		...
		new_animation = "attack"
		if (animation_player.get_current_animation_pos() == animation_player.get_current_animation_length() && current_animation == "attack"):
		...
		exporb.set_value(ep)
		exporb.set_global_pos(Vector2(-608, -416))
		item.set_global_pos(Vector2(-608, -448))
