...
func _fixed_process(delta):
	...
	if (cycle == "appear"):
		...
		currentx += PI/60
		var x = cos(currentx)
		set_global_pos(Vector2(-640 - x * 320, camera.get_global_pos().y - 164))
		# flash the combo
		...
		if (attack_current_cooldown >= ATTACK_COOLDOWN && beam_current_delay < BEAM_DELAY):
			if (randi() % 2 == 0):
				...
				beam.set_pos(Vector2(head.get_pos().x + 1, head.get_pos().y + 48))
				...
				for i in range(0, 6):
					...
					fire.set_global_pos(Vector2(get_global_pos().x + x, get_global_pos().y + y))
					fire.pivot = fire.get_global_pos()
		...
		var deltaX = (current_statue.get_global_pos().x + 16 - get_global_pos().x) / 2.0
		deltaX = sign(deltaX) * min(abs(deltaX), 5)
		var deltaY = (current_statue.get_global_pos().y - 32 - get_global_pos().y) / 2.0
		deltaY = sign(deltaY) * min(abs(deltaY), 5)
		var x = get_global_pos().x + deltaX
		var y = get_global_pos().y + deltaY
		set_global_pos(Vector2(x, y))
		if (abs(x - current_statue.get_global_pos().x - 16) < 1 && abs(y - current_statue.get_global_pos().y + 32) < 1):
			cycle = "docked"
		...
		var x = cos(currentx)
		x = -640 - x * 256
		var deltaX = (x - get_global_pos().x) / 2.0
		deltaX = sign(deltaX) * min(abs(deltaX), 5)
		var y = camera.get_global_pos().y - 164
		var deltaY = (y - get_global_pos().y) / 2.0
		deltaY = sign(deltaY) * min(abs(deltaY), 5)
		set_global_pos(Vector2(get_global_pos().x + deltaX, get_global_pos().y + deltaY))
		if (abs(get_global_pos().x - x) < 1):
			cycle = "display"
	elif (cycle == "dying"):
		# show death animation
		new_animation = "die"
		if (animation_player.get_current_animation_pos() == animation_player.get_current_animation_length() && current_animation == "die"):
		...
		exporb.set_value(ep)
		exporb.set_global_pos(Vector2(-608, -336))
