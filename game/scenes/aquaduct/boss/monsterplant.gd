...
func _fixed_process(delta):
	...
	if (cycle == "attack"):
		...
		if (!playerfound):
			...
			mouth.set_global_pos(Vector2(origin.x + offset.x, origin.y + offset.y))
			...
			if (current_attack_delay >= ATTACK_COOLDOWN):
				...
				projectile.set_global_pos(mouth.get_global_pos())
				...
				projectileleft.set_global_pos(mouth.get_global_pos())
				...
				projectileright.set_global_pos(mouth.get_global_pos())
		check_damage()
	...
	#show weak spot target
		...
		if (target_player.get_current_animation_pos() == target_player.get_current_animation_length()):
			...
			for tile in tiles:
				if (tile.get_name() == "swingboulder"):
					...
					if (current_boulderhp <= 0):
						...
						add_child(target)
						target.set_global_pos(mouth.get_global_pos())
	else:
		current_hurt_delay += 1
