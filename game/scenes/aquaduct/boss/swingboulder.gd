...
func _fixed_process(delta):
	if (state == "idle"):
		...
		accel += 1
		boulder.set_global_pos(Vector2(boulder.get_global_pos().x, boulder.get_global_pos().y + accel))
		if (boulder.get_global_pos().y + 32 >= groundplane || collided):
			state = "dormant"
			collision_rect.set_layer_mask_bit(1, false)
			collision_rect.set_layer_mask_bit(0, false)
			boulder.hide()
			accel = 0
			boulder.set_pos(Vector2(0, 0))
			collided = false
	elif (state == "dormant"):
