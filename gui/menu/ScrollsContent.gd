...
func center_scroll():
	...
	scrollcontainer.set_position(Vector2(-scrollitem.get_position().x - scrollitem.get_size().x / 2 + 24 + scrollcontainerwidth / 2, scrollcontainer.get_position().y))
