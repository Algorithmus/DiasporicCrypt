...
func step(space_state):
	...
	if (!target.get("is_hurt") && !target.get("is_stunned") && !target.get("frozen")):
		...
		if (target.get("ignore_collision")):
			...
			if (target.get_global_position().x + netX >= originalX + target.get("patrolrange") * 32):
				input = "left"
			if (target.get_global_position().x + netX <= originalX):
				input = "right"
		else:
		...
		if (!target.get("ignore_collision")):
			if (change_direction):
				...
				if (target.get("direction") > 0):
					...
					input = "left"
		if (target.can_attack() && (player.get_global_position().y >= target.get_global_position().y - target.get("sprite_offset").y && player.get_global_position().y <= target.get_global_position().y + target.get("sprite_offset").y)):
			var camera = player.get_node("Camera2D")
			var endpos = target.get_global_position().x + target.get("direction") * (target.get("sprite_offset").x - camera.get_offset().x * (1 - player_distance))
			if (player.get_global_position().x * target.get("direction") <= endpos * target.get("direction") && player.get_global_position().x * target.get("direction") >= target.get_global_position().x * target.get("direction")):
				input = "attack"
	else:
		input = ""
