...
func step(space_state):
	...
	if (!target.get("is_hurt") && !target.get("is_stunned") && !player.get("is_hurt") && !target.get("frozen")):
		...
		if (target.get_global_position().x >= camera.get_camera_screen_center().x + camera.get_offset().x && target.get_global_position().x <= camera.get_camera_screen_center().x - camera.get_offset().x):
			if (target.get_global_position().y >= camera.get_camera_screen_center().y + camera.get_offset().y && target.get_global_position().y <= camera.get_camera_screen_center().y - camera.get_offset().y):
				# match vertical position on flying enemies
				if (target.get("ignore_collision")):
					vertical_input = ""
					vertical_inrange = false
					var vertical_direction = 0
					if (target.get_global_position().y > player.get_global_position().y):
						vertical_direction = -1
					elif (target.get_global_position().y < player.get_global_position().y):
						vertical_direction = 1
					if (abs(target.get_global_position().y - player.get_global_position().y) < abs(target.get("direction") * target.get("runspeed") + target.get("gustx"))):
					...
					if (vertical_direction != 0):
						if (vertical_direction > 0):
							...
							vertical_input = "up"
					if (target.get_global_position().y > player.get_global_position().y - player.get("sprite_offset").y && target.get_global_position().y < player.get_global_position().y + player.get("sprite_offset").y):
				...
				if (rush_attack):
					...
					var desired_pos = player.get_global_position().x + preferred_distance * relative_direction
					if (target.get_global_position().x > desired_pos):
						desired_direction = -1
					elif (target.get_global_position().x < desired_pos):
						desired_direction = 1
					# if we're close enough, don't bother closing the gap since the current runspeed will overstep
					# the desired position anyways
					if (abs(target.get_global_position().x - desired_pos) < abs(target.get("direction") * target.get("runspeed") + target.get("gustx"))):
						desired_direction = 0
			...
			if (desired_direction != 0 && !target.get("ignore_collision")):
				...
				var frontX = target.get_global_position().x + sign(netX) * target.get("sprite_offset").x + netX
				...
				var areaTile = space_state.intersect_ray(Vector2(frontX, target.get_global_position().y + target.get("sprite_offset").y - 32), Vector2(frontX, target.get_global_position().y + target.get("sprite_offset").y + 32), target.get("area2d_blacklist"), 2147483647, 16)
