...
func closestXTile_area_check(desired_direction, desiredX, space_state):
	...
	if (frontTile != null && frontTile.has("collider") && !("slope" in frontTile["collider"].get_name()) && !"(player,damage,water,lava,snow,item,damagable,consumable,sensor,magic,switch,breakable,sunbeam,ladder,fake)".match("*" + frontTile["collider"].get_name() + "*") && !frontTile["collider"].has_node("magic") && !("target" in frontTile["collider"].get_parent().get_name())):
		return 0
	return desiredX
